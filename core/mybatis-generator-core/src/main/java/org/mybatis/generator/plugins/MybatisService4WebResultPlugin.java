 ...
 * 2017/11/22 21:40
 */
public class MybatisService4WebResultPlugin extends PluginAdapter
{
	...
	private FullyQualifiedJavaType pb;
	...
	public MybatisService4WebResultPlugin()
	{
		super();
		// 默认是slf4j
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable)
	{
		...
		pb = new FullyQualifiedJavaType("com.cetccity.athena.mybatis.util.PageUtils");
		webResult = new FullyQualifiedJavaType("com.cetccity.athena.common.WebResult");
		 pageList = new FullyQualifiedJavaType("com.cetccity.athena.common.model.mybatis.PageList");
			...
			List<GeneratedJavaFile> files)
	{
		...
		Method method = selectByPrimaryKey(introspectedTable, tableName);
		method.removeAllBodyLines();
		interface1.addMethod(method);

		method = selectAll(introspectedTable, tableName);
		method.removeAllBodyLines();
		interface1.addMethod(method);
	...
	//	sb.append("return this.");
		sb.append(pojoType.getShortName()+" record = ");
		...
		method.addBodyLine("return WebResult.of(record);");
	...
	{
//		FullyQualifiedJavaType pageList = new FullyQualifiedJavaType("com.cetccity.athena.common.model.mybatis.PageList");
		Method method = new Method();
		...
		// method.addBodyLine("try {");
//		sb.append("return this.");
		sb.append(pageList.getShortName()+" list=");
		sb.append(getDaoShort());
		sb.append("selectAll");
		sb.append("(");
		for (IntrospectedColumn introspectedColumn : introspectedTable.getPrimaryKeyColumns())
		{
			sb.append("record");
			sb.append(",");
			sb.append("PageUtils.getPageBounds(");
			sb.append("record");
			sb.append(", \"ID.DESC\")");
			sb.append(",");
			//            sb.append("PageUtils.getPageBounds(subscriber, \"ID.DESC\")");
		}
		sb.setLength(sb.length() - 1);
		sb.append(");");
		method.addBodyLine(sb.toString());
		method.addBodyLine("return WebResult.of(list);");
		// method.addBodyLine("} catch (Exception e) {");
		// method.addBodyLine("logger.error(\"Exception: \", e);");
		// method.addBodyLine("return null;");
		// method.addBodyLine("}");
...
//		sb.append("return this.");
		...
		method.addBodyLine(sb.toString());
		method.addBodyLine("String[] logparam = { \"\", \"\" };");
		method.addBodyLine("return WebResult.addLogdada(WebResult.ok(\"S.message!!!!请修改\"), \"L.message!!!!请修改\", logparam);");
		return method;
	}
...
//		}
	...
	protected Method getOtherInsertboolean2(String methodName, IntrospectedTable introspectedTable, String tableName)
	{
		Method method = new Method();
		method.setName(methodName);
		method.setReturnType(webResult);
	...
	private void addImport(Interface interfaces, TopLevelClass topLevelClass)
	{
		...
		interfaces.addImportedType(webResult);
		topLevelClass.addImportedType(daoType);
		...
		topLevelClass.addImportedType(pb);
