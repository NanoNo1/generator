...
public class DatabaseIntrospector
{
			...
			TableConfiguration tc) throws SQLException
	{
		...
		while (rs.next())
		{
			...
			//jiangxp 增加代码为了remark字段注释 start
			...
			{
				introspectedColumn
						.setAutoIncrement("YES".equals(rs.getString("IS_AUTOINCREMENT"))); //$NON-NLS-1$ //$NON-NLS-2$
			}
				...
				.entrySet())
		{
			...
			if ("MYSQL".equals(databaseProductName))
			{
				...
				ResultSet rs = stmt.executeQuery(
						new StringBuilder().append("SHOW TABLE STATUS LIKE '").append(atn.getTableName()).append("'")
								.toString());
				while (rs.next())
